  ...
  class CleanerImprover < BaseImprover
    ...
    def system_prompt
      <<~TXT
        ...
        Principe "Dites-le nous une fois" (DLNUF) : l'administration ne doit pas redemander des informations déjà collectées soit à l'entrée du formulaire,
        soit par un champ qui collecte des informations complémentaires.
    ...
    def schema_prompt
      <<~TXT
        Voici le schéma des champs demandés à l'usager.
        ...
        - annuaire_education : Identifiant d'un établissement scolaire. Fournit : nom de l'étabissement, adresse normalisée, académie, nature de l'établissement, téléphone, email, site internet.
    ...
    def rules_prompt
      <<~TXT
        ## Règles :
        - Identifie les champs qui demandent une information déjà disponible via un autre champ du formulaire
        plus haut dans la démarche.
